                ...
                dbUpdateException.InnerException is SqlException sqlException)
            {
                foreach (SqlError error in sqlException.Errors)
                {
                    ...
                    if (error.Number == 2601 || error.Number == 2627)
                    {
                        ...
                        if (msg.Contains("UX_Complaints_Student_Pending_Ticket"))
                            throw new DuplicateBookingException("You already have an identical pending complaint ticket submitted for this category.");
